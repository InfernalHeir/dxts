...
const withdrawalROI = async (user_info, amountWithdrawn) => {
  try {
    ...
    console.log("--withdrawalROI----user_level_percentage", user_level_percentage);
    ...
    for (let index = 0; index < user_level_percentage.length; index++) {
      ...
      console.log("-----withdrawalROI-----user.user_application_wallet", user.user_application_wallet);
      console.log("--withdrawalROI----(amount_Withdrawn * user.percentage)", amount_Withdrawn * user.percentage);
...
const getpendingwithdrawalList = async (req, res) => {
  ...
  for (var i = 0; i < withdrawalusers.length; i++) {
    ...
    withdrawalusers[i]["requesteduseruuid"] = userinfo.uuid, withdrawalusers[i]["trxaddress"] = userinfo.trx_user_walletaddress;
  }
  return res.json({
    status: true,
...
const updatependingwithdrawalstatus = async (req, res) => {
  ...
  if (!description || description.length <= 0) return res.status(500).json({
    status: false,
    message: "Please provide a description"
  });

  if (!txid || txid.length <= 0) return res.status(500).json({
    status: false,
    message: "Please provide a txid "
  });

  if (!withdrawreqId || withdrawreqId.length <= 0) return res.status(500).json({
    status: false,
  ...
  if (withdrawExist.length <= 0) return res.status(500).json({
    status: false,
