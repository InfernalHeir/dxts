...
var web3 = new Web3(new Web3.providers.HttpProvider("https://mainnet.infura.io/v3/d2a4bb4e9b864b74bed287ba67ceebb5", null, options)); // process.env.ETHNODE_URL
...
const ERROR_PENDINGTRANSACTION = "Error: Returned error: replacement transaction underpriced";
const ERROR_UNANLETOFETCHTRANSACTIONRECEIPT = "Error: Failed to check for transaction receipt:";
...
const tranferTokens = async (receiverAddress, tokenAmount, senderAddress, PRIVATE_KEY_SENDER, isprivatekey) => {
  let txId;
  ...
  try {
    ...
    if (!senderTokenBalance < tokenAmount) {
      await contract.methods.transfer(receiverAddress, tokenAmount).estimateGas({ from: senderAddress }, function (error, gasAmount) {
      ...
      await contract.methods.transfer(receiverAddress, tokenAmount).send({ from: senderAddress, gas: gasRequired }).on("transactionHash", function (transactionHash, error) {
        console.log("Transaction Hash" + transactionHash);
        txId = transactionHash;
      }).catch(error => {
        if (error.toString() == ERROR_PENDINGTRANSACTION) {
          console.log("sender have already a transaction in pending state please retry after some time ");
        }
        if (error.toString().includes(ERROR_UNANLETOFETCHTRANSACTIONRECEIPT)) {
          console.log("unable to fetch the transaction receipt please check the status from transaction id stored in database ");
        }
        return {
          status: false,
          message: "Something went wrong!"
...
var tokenAmountEqUSDdeposit = async (req, res) => {
  try {
    ...
    var response = await axios.get("https://api.coingecko.com/api/v3/coins/destiny-success");
...
var userWalletExist = async uuid => {
  ...
  if (!user_wallet_Exist.dataValues.trx_user_walletaddress || user_wallet_Exist.dataValues.length <= 0) {
    return {
      status: false
    };
  }
  var tokenBalance = await getDxtsBalance(user_wallet_Exist.dataValues.trx_user_walletaddress);
...
var tokenBignumberConversion = async tokenAmount => {
  try {
    ...
    console.log("--inside ---conversion---amountToSendinDecimal", amountToSendinDecimal);
    return amountToSendinDecimal;
  } catch (e) {
    console.log("---e---", e);
  }
};
const transferEthers = async (receiverAddress, senderAddress, ethAmount, PRIVATE_KEY_USER_SENDER, isprivatekey, ADMIN_TOKEN_TRANSFER_ADDR, ADMIN_TOKEN_TRANSFER_ADDR_PY_KEY, tokenAmount, userId) => {
  var iscorrecReceiverAddress = await checkAddress(receiverAddress);
  var iscorrecSenderAddress = await checkAddress(senderAddress);
  ...
  web3.eth.sendTransaction({
    ...
    var txinfo;
    if (receipt.status) {
      await userEthmanagmentmodel.update({ ether_tx_status: receipt.status }, { where: { ether_transfer_txid: transactionHash } });

      txinfo = await tranferTokens(senderAddress, tokenAmount, ADMIN_TOKEN_TRANSFER_ADDR, ADMIN_TOKEN_TRANSFER_ADDR_PY_KEY, isprivatekey = true);
    } else {
