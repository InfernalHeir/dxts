...
const montlyInvestment = async userId => {
  console.log("---------------", userId);
  console.log("---------------", moment(moment().add(30, "days")).format("YYYY-DD-MM"));
  ...
  for (var i = 0; i < montlyincome_wallets.length; i++) {
    if (montlyincome_wallets[i].type == COLDWALLET) coldwallet_percentage = montlyincome_wallets[i].percentage;

    if (montlyincome_wallets[i].type == PRINCIPALAMOUNT) principale_percentage = montlyincome_wallets[i].percentage;

    if (montlyincome_wallets[i].type == PROFITBONOUS) profitbonous_percentage = montlyincome_wallets[i].percentage;
  }
  ...
  var amount_already_received = parseFloat(pricipalamoutTotal[0].pricipalamoutTotal + profitbonousamountTotal[0].profitbonousamountTotal + coldwalletamountTotal[0].coldwalletamountTotal);
  ...
  var monthly_payemnt = await db.sequelize.query("INSERT INTO Monthly_returns (Monthly_return_percentage,principal_returns_wallet,profit_bonus_wallet,cold_wallet ,userId ,nextpaymentDate,createdAt,updatedAt) values (:Monthly_return_percentage , :principal_returns_wallet, :profit_bonus_wallet , :cold_wallet , :userId, STR_TO_DATE(:nextpaymentDate , '%Y-%m-%d'),STR_TO_DATE(:createdAt , '%Y-%m-%d'),STR_TO_DATE(:updatedAt , '%Y-%m-%d')  )", {
    replacements: {
      Monthly_return_percentage: parseFloat(principale_percentage + profitbonous_percentage + coldwallet_percentage),
...
const receiveTodaysInvestment = async () => {
  var todaysReceiversList = await db.sequelize.query("select distinct userId as userId  from Monthly_returns where  DATE(NOW())= DATE(nextpaymentDate) ", {
    type: QueryTypes.SELECT
  });
