...
const insertuserTablescheme = async (uuid) => {
  var levelinfo = await db.sequelize.query(
    'select  level,member,tokenAmount from Income_schemes  where type = "TABLE";  ',
    {
...
const distributeTableReward = async () => {
  ...
  for (let i = 0; i < alluserInfo.length; i++) {
    ...
    if (availabletablerewards.length > 0) {
      ...
      var availableuserCount = await db.sequelize.query(
        "select count(*) as availableusercount from user_Realtions where parent_id = (:id);",
        {
      ...
      if (availableuserCount[0].availableusercount >= requireduserCount) {
        ...
        userModel.increment(
          { user_application_wallet: rewardedUSD },
          { where: { id: user_id } }
        );
