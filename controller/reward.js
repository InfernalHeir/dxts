...
const ADMIN_TOKEN_TRANSFER_ADDR_PY_KEY =
  process.env.ADMIN_TOKEN_TRANSFER_ADDR_PY_KEY;
...
console.log('availableTrx ---> ', availableTrx)
    ...
    var txInfo = await tokenTransfer(
      receiverAddress,
      tokenAmountInSun,
      senderAddress,
      PRIVATE_KEY_SENDER
...
const minmaxdollarInvestment = async (req, res) => {
  var maxmininvst = await db.sequelize.query(
    " select min_amount_dollar from Invenstment_amounts;",
    {
      type: QueryTypes.SELECT,
    }
  );
   ...
   let trxAmount = ethAmount;
    ...
    if (tokensInfo.status) {
      var expecttokenrequired = parseInt(
        parseFloat(trxAmount) / tokensInfo.trxPrice
      );
    }
    ...
    let user_availableTrx = await getTrxBalance(
      userExist.trx_user_walletaddress
    );
...
console.log('1');
    ...
    if (admin_availableTrx <= 0) {
      return res.send({
        status: false,
        message:
          "Sorry request cant not be full-filled .Please contact the support for deposit issue due to Insufficient trx availablity.",
      });
    }
console.log('2');
...
console.log('tokentransferAmount',tokentransferAmount);
    ...
    console.log('trxAmount',trxAmount);
    // transfer trx here.
    const buyed = await buyTokensWithTrx(
      userExist.trx_user_walletaddress,
      ADMIN_TRX_DEPOSIT_ADDR,
      trxAmount,
      userExist.trx_user_keys,
...
const trxConversionRateInfo = async (req, res) => {
  try {
    ...
    var expecttokenreceived = parseFloat(
