...
const generatesampleuuid = async (req, res) => {
  ...
  var randomId = generateRerferalcode(
...
const registration = async (req, res) => {
  try {
    ...
    if (!withdrawalpassword || withdrawalpassword.length <= 0)
      return res.status(500).json({
        status: false,
    ...
    if (
      user_alreadyExist &&
      user_alreadyExist.dataValues &&
      Object.keys(user_alreadyExist.dataValues).length > 0
    ) {
      return res.status(500).json({
        status: false,
...
const dashBoardInfo = async (req, res) => {
  ...
  var pricipalamoutTotal = await db.sequelize.query(
    "select IFNULL(sum( principal_returns_wallet ) , 0)  as pricipalamoutTotal from Monthly_returns where userId = (:id) AND isprincipalWithdrawal  = false   ",
    {
      replacements: { id: userExist.id },
      type: QueryTypes.SELECT,
    }
  ...
  var profitbonousamountTotal = await db.sequelize.query(
    "select IFNULL(sum( profit_bonus_wallet ), 0) as profitbonousamountTotal from Monthly_returns where userId = (:id) AND isprofitBonousWithdrawal = false  ",
    {
      replacements: { id: userExist.id },
      type: QueryTypes.SELECT,
    }
  ...
  var withdrawalIncome = await db.sequelize.query(
    "select IFNULL(sum( dollaramountWithdraw ), 0) as dollaramountWithdraw from WithdrawalRequests where requesteduserId = (:id) AND status = (:WITHDRAWN) ",
    {
...
const admindashBoardInfo = async (req, res) => {
  ...
  var sumreferalIncome = await db.sequelize.query(
    ' select sum(rewardAmount) as rewardAmount from Rewards where  type = "REFERRAL"',
    {
      type: QueryTypes.SELECT,
    }
  );
  ...
  var pricipalamoutTotal = await db.sequelize.query(
    "select IFNULL(sum( principal_returns_wallet ) , 0)  as pricipalamoutTotal from Monthly_returns  ",
    {
      type: QueryTypes.SELECT,
    }
  );
  ...
  var coldwalletamountTotal = await db.sequelize.query(
    "select IFNULL(sum( cold_wallet ), 0) as coldwalletamountTotal from Monthly_returns ",
    {
      type: QueryTypes.SELECT,
    }
  );
  ...
  console.log("coldwalletamountTotal", coldwalletamountTotal);
  var profitbonousamountTotal = await db.sequelize.query(
    "select IFNULL(sum( profit_bonus_wallet ), 0) as profitbonousamountTotal from Monthly_returns ",
    {
      type: QueryTypes.SELECT,
    }
  );
  ...
  var withdrawalIncome = await db.sequelize.query(
    "select IFNULL(sum( dollaramountWithdraw ), 0) as dollaramountWithdraw from WithdrawalRequests where  status = (:WITHDRAWN) ",
    {
...
const usertotalIncome = async (req, res) => {
  ...
  var availabletablerewards = await rewardModel.findAll({
    attributes: [
      ...
      ["TableIncome_Singletokenrpice", "tokenPrice"],
    ],
    raw: true,
    where: {
      isWithdrawal: true,
  ...
  for (let index = 0; index < availabletablerewards.length; index++) {
    ...
    totalTableIncome =
      totalTableIncome + tableIncome["tokenAmount"] * tableIncome["tokenPrice"];
  }

  finalIncome = parseFloat(
    sumreferalIncome +
      monthlyIncome +
      withdrawalIncome +
      totalTableIncome +
...
const profitbonuswalletWithdrawal = async (req, res) => {
  ...
  var profitbonouswalletamountTotal = await db.sequelize.query(
    "select IFNULL(sum( profit_bonus_wallet ), 0) as profitbonouswalletamountTotal from Monthly_returns where userId = (:id)  AND isprofitBonousWithdrawal = false ",
    {
      replacements: { id: userExist.id },
      type: QueryTypes.SELECT,
    }
  ...
  console.log(
    "profitbonouswalletamountTotal",
    profitbonouswalletamountTotal[0].profitbonouswalletamountTotal
  );
  ...
  var profitbonous_monthlty_reward_ids = await db.sequelize.query(
    "select id from Monthly_returns where userId = (:id)  AND isprofitBonousWithdrawal = false ",
    {
      replacements: { id: userExist.id },
      type: QueryTypes.SELECT,
    }
  ...
  console.log(
    "profitbonous_monthlty_reward_ids",
    profitbonous_monthlty_reward_ids
  );
  ...
  var finalwalletAmount = parseFloat(
    userExist.user_application_wallet +
      profitbonouswalletamountTotal[0].profitbonouswalletamountTotal
  );
...
const principalwalletWithdrawal = async (req, res) => {
  ...
  var principalwalletamountTotal = await db.sequelize.query(
    "select IFNULL(sum( principal_returns_wallet ), 0) as principalwalletamountTotal from Monthly_returns where userId = (:id)  AND isprincipalWithdrawal = false ",
    {
      replacements: { id: userExist.id },
      type: QueryTypes.SELECT,
    }
  ...
  console.log(
    "principalwalletamountTotal",
    principalwalletamountTotal[0].principalwalletamountTotal
  );
  ...
  var principal_monthlty_reward_ids = await db.sequelize.query(
    "select id from Monthly_returns where userId = (:id)  AND isprincipalWithdrawal = false ",
    {
      replacements: { id: userExist.id },
      type: QueryTypes.SELECT,
    }
...
const coldwalletWithdrawal = async (req, res) => {
  ...
  var coldrewarddate = await db.sequelize.query(
    "select createdAt from Monthly_returns where  userId = (:id)  LIMIT 1  ",
    {
      replacements: { id: userExist.id },
      type: QueryTypes.SELECT,
    }
  ...
  var startDate = moment(coldrewarddate[0].createdAt, "YYYY-MM-DD").format(
    "YYYY-MM-DD"
  );
  ...
  var coldwalletamountTotal = await db.sequelize.query(
    "select IFNULL(sum( cold_wallet ), 0) as coldwalletamountTotal from Monthly_returns where userId = (:id)  AND iscoldWithdrawal = false ",
    {
      replacements: { id: userExist.id },
      type: QueryTypes.SELECT,
    }
  ...
  var cold_monthlty_reward_ids = await db.sequelize.query(
    "select id from Monthly_returns where userId = (:id)  AND iscoldWithdrawal = false ",
    {
      replacements: { id: userExist.id },
      type: QueryTypes.SELECT,
    }
...
async function getAllMuliLevelUserList() {
  var LevelwiseAllUserList = await db.sequelize.query(
    "SELECT `level`,`child_id`,`user_Realtions`.`parent_id`,`uuid` As `child_uuid` FROM `user_Realtions` inner join Users WHERE `user_Realtions`.`child_id` = `Users`.`id` ",
    {
      type: QueryTypes.SELECT,
    }
  );
  ...
  for (let i = 0; i < LevelwiseAllUserList.length; i++) {
    ...
    let parentObj = alluserInfo.find(
      (x) => x.id == levelwiseuserinfo.parent_id
    );
...
async function getMuliLevelUsers(user_id) {
  var LevelwiseUserList = await db.sequelize.query(
    "SELECT `level`, `child_id`,`uuid` as `child_uuid` ,`isActive` ,`account_status`,`Users`.`parent_id` As  `sponserId`   FROM `user_Realtions` inner join Users WHERE   `user_Realtions`.`parent_id` = (:id) AND `user_Realtions`.`child_id` = `Users`.`id`  ORDER BY `level` ASC",
    {
...
function generateRerferalcode(uuid) {
  return String(uuid.substring(0, 2)).concat(
    String(Date.now()).substring(2, 8)
  );
}
